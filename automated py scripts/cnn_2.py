...
Original file is located at
    https://colab.research.google.com/drive/1UoVtoySuMfpYPbBLNHD60Zo2eXAeu5en
"""
...
for i in range(1,32,2):
    ...
    Qbs = series_to_supervised(Qb,i,3)
    ...
    Input = Input.drop(Input.index[-2:])

    Input.columns

    y=Input[Input.columns[Input.columns.isin(['var(t+2)2'])]]

    y

    x=Input.drop(['var(t+1)2','var(t+2)2'],axis=1)
    ...
    # model_cnn.summary()
    model_cnn = Sequential()
    model_cnn.add(Conv1D(filters=5, kernel_size=2, activation='relu', input_shape=(x_train.shape[1], x_train.shape[2])))
    ...
    history = model_cnn.fit(x_train, y_train, validation_data=(x_val, y_val), epochs=200, verbose=1,batch_size = 16)
