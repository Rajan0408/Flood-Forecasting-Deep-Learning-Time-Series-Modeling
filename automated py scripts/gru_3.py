...
Original file is located at
    https://colab.research.google.com/drive/1lYcGhmoIQ2gdQWHkRymxDyj0CIhsKO5M
"""
...
for i in range(1,32,2):
    ...
    regressorGRU.add(GRU(units=10, return_sequences=True, input_shape=(x_train.shape[1],x_train.shape[2]), activation='softsign'))
    regressorGRU.add(Dropout(0.2))
    regressorGRU.add(Dense(units=1))
    # Compiling the RNN
    regressorGRU.compile(optimizer='adam',loss='mae')
    # Fitting to the training set
    GRU_history =regressorGRU.fit(x_train,y_train,validation_data=(x_val, y_val),epochs=200,batch_size=16)

    hist_df = pd.DataFrame(GRU_history.history )
    hist_df

    plt.plot(GRU_history.history['loss'],label='Training loss' )
    plt.plot(GRU_history.history['val_loss'], label='Testing loss')
    plt.xlabel('no_of_epochs', fontsize=8 )
    plt.ylabel('Training Loss', fontsize=8 )
    plt.legend()

    train_yp = regressorGRU.predict(x_train)
    val_yp = regressorGRU.predict(x_val)
    test_yp =regressorGRU.predict(x_test)
    ...
    y_lim = plt.ylim([0, 100])
    x_lim = plt.ylim([0, 100])
    ...
    res.to_csv(str(i)+'gru.csv')

from google.colab import files
#import pandas as pd
#result.to_csv('example_file.csv')
for i in range(1,32,2):
  files.download(str(i)+'gru.csv')
